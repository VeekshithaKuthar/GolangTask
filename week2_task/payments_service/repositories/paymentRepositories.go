...
type IPaymentDB interface {
	...
	UpdatePaymentStatus(order_Id uint, paymentstatus string) error
}

func (udb *PaymentOrderDb) UpdatePaymentStatus(order_Id uint, paymentstatus string) error {
	tx := udb.DB.Model(&models.Payments{}).
		Where("order_id= ?", order_Id).Update("paymentStatus", paymentstatus)
	if tx.Error != nil {
		return tx.Error
	}
